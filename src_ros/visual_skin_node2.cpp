...
#endif
//-------------------------------------------------------------------------------------------
#include "ay_vision_msgs/BlobMoves.h"
#include "ay_vision_msgs/ProxVision.h"
  ...
  while(!Shutdown)
  {
    if(Running)
    {
      ...
      // Publish as BlobMoves
      {
        ...
        ay_vision_msgs::BlobMoves blob_moves;
        ...
        {
          ay_vision_msgs::BlobMove &m(blob_moves.data[i]);
          m.Pox= itr->Po.x;
          m.Poy= itr->Po.y;
          m.So = itr->So;
          m.DPx= itr->DP.x;
  ...
  while(!Shutdown)
  {
    if(Running)
    {
      ...
      // Publish as PXVPub
      {
        ...
        ay_vision_msgs::ProxVision prox_vision;
...
int main(int argc, char**argv)
{
  ...
  for(int j(0),j_end(BlobTracker.size());j<j_end;++j)
    BlobPub[j]= node.advertise<ay_vision_msgs::BlobMoves>(std::string("blob_moves_")+CamInfo[j].Name, 1);

  PXVPub.resize(ObjDetTracker.size());
  for(int j(0),j_end(ObjDetTracker.size());j<j_end;++j)
    PXVPub[j]= node.advertise<ay_vision_msgs::ProxVision>(std::string("prox_vision_")+CamInfo[j].Name, 1);
